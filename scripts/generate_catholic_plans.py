...
BOOK_NAMES_EN = {
    ...
    67: "Tobit", 68: "Judith", 69: "Wisdom", 70: "Sirach",
    71: "Baruch", 72: "1 Maccabees", 73: "2 Maccabees"
}
...
BOOK_BLESSINGS = {
    ...
    67: {"ref": "Tobit 13:14", "ta": "உன்னை நேசிப்போரும் உன் அமைதியில் மகிழ்வோரும் பேறுபெற்றோர்.", "en": "Blessed are all they that love thee, and that rejoice in thy peace."},
    69: {"ref": "Wisdom 3:9", "ta": "ஆண்டவரை நம்பியோர் உண்மையை அறிந்துகொள்வர்.", "en": "They that put their trust in him shall understand the truth."},
    70: {"ref": "Sirach 2:10", "ta": "முந்தைய தலைமுறைகளை எண்ணிப்பாருங்கள்; ஆண்டவரை நம்பி ஏமாற்றம் அடைந்தவர் யாராவது உண்டா?", "en": "Look at the generations of old, and see; did ever any trust in the Lord, and was confounded?"}
}
...
def get_chapters(book_ids):
    ...
    cur.execute(f"""
        ...
        WHERE b.id IN ({placeholders}) AND v.chapter >= 1
...
def split_into_days(chapters, total_target_days, plan_id, plan_name_en, plan_name_ta):
    total_words = sum(c["word_count"] for c in chapters)
    days = []
    curr_idx = 0
    num_chapters = len(chapters)

    for day_num in range(1, total_target_days + 1):
        if curr_idx >= num_chapters:
            break
            
        remaining_days = total_target_days + 1 - day_num
...
def main():
    ...
    # 1. Deuterocanonical Books in 30 Days (137 chapters, books 67 to 73)
    print("Generating Deuterocanonical 30-Day Plan...")
    dc_chapters = get_chapters(list(range(67, 74)))
    dc_plan = split_into_days(
        dc_chapters, 30,
        "plan_30_deuterocanon",
        "30-Day Deuterocanonical Books Read-Along",
        "30 நாட்கள் இரண்டாம் திருமுறை வாசிப்பு"
    )
    dc_file = os.path.join(OUT_DIR, "plan_30_deuterocanon.json")
    with open(dc_file, "w", encoding="utf-8") as f:
        json.dump(dc_plan, f, ensure_ascii=False, indent=2)
    print(f"Generated: {dc_file} ({len(dc_plan['days'])} days, {len(dc_chapters)} chapters)")

    # 2. Entire Catholic Bible in 100 Days (1326 chapters, books 1 to 73)
    print("Generating Entire Catholic Bible 100-Day Plan...")
    cat_chapters = get_chapters(list(range(1, 74)))
    cat_plan = split_into_days(
        cat_chapters, 100,
        "plan_100_catholic_bible",
        "100-Day Catholic Bible Read-Along",
        "100 நாட்கள் கத்தோலிக்க திருவிவிலிய வாசிப்பு"
    )
    cat_file = os.path.join(OUT_DIR, "plan_100_catholic_bible.json")
    with open(cat_file, "w", encoding="utf-8") as f:
        json.dump(cat_plan, f, ensure_ascii=False, indent=2)
    print(f"Generated: {cat_file} ({len(cat_plan['days'])} days, {len(cat_chapters)} chapters)")
